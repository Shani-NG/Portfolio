  ...
  return {
    ...
    async generateReport(input: RoleFitModelInput): Promise<RoleFitModelResult> {
      ...
      const reportPrompt = [
        ...
        "Before presenting a missing direct domain, tool, or industry match as an isolated gap, check whether approved evidence provides genuinely relevant semantic or operationally transferable context. When it does, state the direct gap, the transferable context, and the residual unproven gap in the same item; do not claim the direct domain/tool/industry is evidenced, do not equate adjacent domains, and do not improve fitLevel solely because transferable context exists.",
