  ...
  return {
    ...
    async generateReport(input: RoleFitModelInput): Promise<RoleFitModelResult> {
      ...
      const reportPrompt = [
        ...
        "Use only exact APPROVED_SOURCE_ID values supplied in Retrieved Approved Evidence. ROLE_ITEM_CANDIDATE_SOURCE_IDS are ranked suggestions for that roleItemIndex, not an authorization boundary. Prefer the strongest truthful public case-study evidence, prefer an unused project only when alternatives are similarly relevant, never reuse the same evidence item, and use CV evidence only when no sufficiently relevant case-study evidence supports the requirement. Positive or partial matches require at least one supporting evidenceSourceId; gaps may use an empty list.",
