  ...
  return {
    ...
    async generateReport(input: RoleFitModelInput): Promise<RoleFitModelResult> {
      ...
      const reportPrompt = [
        ...
        "Use only exact APPROVED_SOURCE_ID values supplied in Retrieved Approved Evidence, and only IDs listed in ROLE_ITEM_CANDIDATE_SOURCE_IDS for that roleItemIndex. A public case-study source is authoritative whenever it supports the claim; CV/profile evidence is a fallback only when no case-study source supports that claim. Positive or partial matches require at least one supporting evidenceSourceId; gaps may use an empty list.",
